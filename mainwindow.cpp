...
void MainWindow::bringToFront()
{
    ...
    for (const QGraphicsItem *item : overlapItems) {
        if (item->zValue() >= zValue && item->type() == DiagramItem::Type)
            zValue = item->zValue() + 0.1;  //find the max zValue and add 0.1
    }
    selectedItem->setZValue(zValue);//set selectedItem's Value.
}
//! [5]
...
//! [21]
    ...
    buttonGroup->addButton(textButton, InsertTextButton);//set the index of the InsertTextButton
...
//! [23]
    ...
    QPixmap pixmap(":/images/bold.png");//load image
...
QWidget *MainWindow::createCellWidget(const QString &text, DiagramItem::DiagramType type)
{
    ...
    button->setIconSize(QSize(50, 50));
    button->setCheckable(true);
    buttonGroup->addButton(button, int(type));//int(type):set the button's index of the QButtonGroup
